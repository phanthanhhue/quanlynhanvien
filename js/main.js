...
function FormValidation() {
    ...
    // Kiem tra Ho
    isValid &= KiemTraRong('ho', 'thongBaoHo', 'Vui long nhap ho') && KiemTraDinhDangChu('ho', 'thongBaoHo', 'Ho phai la chu');
    // isValid &= true
    // isValid = isValid && true

    // Kiem tra Ten
    isValid &= KiemTraRong('ten', 'thongBaoTen', 'Vui long nhap ten') && KiemTraDinhDangChu('ten', 'thongBaoTen', 'Ten phai la chu');


    isValid &= KiemTraMa(8, 10);
...
function themNhanVien() {
    if (FormValidation()) {
        ...
        nhanVienSV.themNhanVien().then(function (res) {
            console.log(res);
...
function xoaNhanVien(index) {
    Swal({
        ...
        if (result.value) {
            axios({
                method: "DELETE",
                url: `https://5bd2959ac8f9e400130cb7e9.mockapi.io/api/${DSNV[index].MaNV}`,
            })
                .then(function (res) {
                    console.log(res);
                    DSNV.splice(index, 1);
...
function CapNhatNhanVien() {
    ...
    axios({
        method: "PUT",
        url: `https://5bd2959ac8f9e400130cb7e9.mockapi.io/api/nhanVien/${nhanVienCapNhat.MaNV}`,
